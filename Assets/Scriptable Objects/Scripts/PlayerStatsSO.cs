...
public class PlayerStatsSO : ScriptableObject
{
    ...
    {
        EnsureLookup();
        return _statLookup.TryGetValue(type, out var entry) ? entry.BaseValue : 0f;
    }

    public float GetBaseIncreaseAmount(StatType type)
    {
        EnsureLookup();
        return _statLookup.TryGetValue(type, out var entry) ? entry.BaseIncreaseAmount : 0f;
    }

    public string GetName(StatType type)
    {
        EnsureLookup();
        return _statLookup.TryGetValue(type, out var entry) ? entry.Name : string.Empty;
    }

    public Sprite GetIcon(StatType type)
    {
        EnsureLookup();
        return _statLookup.TryGetValue(type, out var entry) ? entry.Icon : null;
    }
